...
class Tokenizer(object):
    ...
    urls_re = re.compile(r"(?:(?:https?\://[A-Za-z0-9\.]+)|(?:(?:www\.)?[A-Za-z0-9]+\.(?:{})))(?:\/\S+)?"
                         "(?=\s+|$)".format(domains))
    del domains
    custom_hashtags_re = re.compile(r"[A-Z][a-z]+(?=[A-Z|\d|\w]?)|[A-Z]+(?=[A-Z][a-z]|\d|\s)|(?<=\d)+[nrt][dh]|(?<=[#\s\d])[a-z]+(?=[A-Z]|\d|\w])|(?<=[\w]-)\w+")
    ...
    del number_re
    other_re = r"(?:[^#\s\.]|\.(?!\.))+"
    _token_regexs = ('custom_hashtags', 'usernames', 'urls', 'times', 'phonenumbers', 'numbers')
    ...
    custom_repeating_patternt_re = re.compile(r"([a-zA-Z][aeiou]|[a-zA-Z][aeiou][a-zA-Z]){1}\1\1+")
    doublequotes = ((u'“',u'”'),(u'"',u'"'),(u'‘',u'’'),(u'＂',u'＂'))
    punctuation = (u'!$%()*+,-/:;<=>?[\\]^_.`{|}~\'' + u''.join(c for t in doublequotes for c in t))
    quotes_re = re.compile(u"|".join(u'({}.*?{})'.format(f,s) for f,s in doublequotes) + u'|\s(\'.*?\')\s')
    del doublequotes
